...
import { formatUnixTimestampMs, formatUnixTimestampMsOrDefault } from "./formatUnixTimestamp";
...
describe(formatUnixTimestampMsOrDefault.name, () => {
    ...
    zip(inputs, outputs).forEach(([input, output]) => {
        it(`Parses correct ${output}`, () => {
            expect(formatUnixTimestampMsOrDefault(input!, "UNREACHABLE")).toEqual(output!)
        })
    })
    ...
    it('Parses incorrect and returns the default value', () => {
        ...
        expect(formatUnixTimestampMsOrDefault(null, defaultValue)).toEqual(defaultValue)
    })
})
