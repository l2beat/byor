import { Transaction, EthereumAddress, Unsigned64 } from "./Transactions"
...
describe('serialize', function() {
    ...
    const SERIALIZED_MSG_SIZE = (20 + 8 + 8 + 8 + 32 + 32 + 1 + 32);
    it('Should serialize a valid transaction', async function() {
        const serialized = await serialize(tx, account);
        expect(serialized.length).to.equal(SERIALIZED_MSG_SIZE * 2 + 2);
