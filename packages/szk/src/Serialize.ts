...
import { Transaction, Unsigned64, Hex, SIGNED_TX_HEX_SIZE } from "./Transactions"
...
export async function deserialize(signedTxBytes: Hex): Promise<E.Either<Transaction, Error> > {
    if(signedTxBytes.length !== SIGNED_TX_HEX_SIZE) {
        return E.right(new Error(`Serialized transaction byte array too small, got/expected => ${signedTxBytes.length}/${SIGNED_TX_HEX_SIZE}`))
    }
