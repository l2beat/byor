...
describe(TransactionExecutor.name, () => {
  describe(TransactionExecutor.prototype.executeBatch.name, () => {
    it('executes an empty transaction to update the nonce of the sender only', async () => {
      ...
      executor.executeBatch(batch, batchPoster)
      const state = executor.finalize()
    ...
    it('throws on zero nonce', async () => {
      ...
      expect(() => executor.executeBatch(batch, batchPoster)).toThrow()
    })
